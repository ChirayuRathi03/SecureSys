...
task_no = "4.2.22"
expected_output = "CRYPTO_POLICY="
output = []
actual_output = []
try:
    forward_output = subprocess.run("grep -Pi '^\\h*CRYPTO_POLICY\\h*=' /etc/sysconfig/sshd", shell=True, capture_output=True, text=True).stdout.strip()
    if forward_output:
        actual_output = forward_output
        if actual_output.lower().startswith(expected_output.lower()):
            true_counter += 1
            true_tasks.append(task_no)
            task_data.append([task_no, "True", expected_output, actual_output])
        else:
