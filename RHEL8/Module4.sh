...
# Task 4.1.1.1: Ensure cron daemon is enabled and active
...
# Task 4.1.1.2: Ensure permissions on /etc/crontab are configured
chown root:root /etc/crontab
chmod og-rwx /etc/crontab

# Task 4.1.1.3: Ensure permissions on /etc/cron.hourly are configured
chown root:root /etc/cron.hourly/
chmod og-rwx /etc/cron.hourly/

# Task 4.1.1.4: Ensure permissions on /etc/cron.daily are configured
chown root:root /etc/cron.daily/
chmod og-rwx /etc/cron.daily/

# Task 4.1.1.5: Ensure permissions on /etc/cron.weekly are configured
chown root:root /etc/cron.weekly/
chmod og-rwx /etc/cron.weekly/

# Task 4.1.1.6: Ensure permissions on /etc/cron.monthly are configured
chown root:root /etc/cron.monthly/
chmod og-rwx /etc/cron.monthly/

# Task 4.1.1.7: Ensure permissions on /etc/cron.d are configured
chown root:root /etc/cron.d/
chmod og-rwx /etc/cron.d/

# Task 4.1.1.8: Ensure crontab is restricted to authorized users
...
# Task 4.1.2.1: Ensure at is restricted to authorized users:
...
# Task 4.2.1 Ensure permissions on /etc/ssh/sshd_config are configured
{
...
# Task 4.2.2: Ensure permissions on SSH private host key files are configured
...
# Task 4.2.3 Ensure permissions on SSH public host key files are configured
output=""
output2=""
pmask="0133"
maxperm=$(printf '%o' $((0777 & ~$pmask)))
...
# Task 4.2.4: Ensure sshd Ciphers are configured
...
# Task 4.2.5: Ensure sshd KexAlgorithms is configured
...
# Task 4.2.6: Ensure sshd MACs are configured
...
# Task 4.2.7: Ensure sshd access is configured
prompt_for_input() {
...
systemctl reload-or-restart sshd
echo "16"
# Task 4.2.8: Ensure sshd Banner is configured
SSHD_CONFIG="/etc/ssh/sshd_config"
...
# Task 4.2.9: Ensure sshd ClientAliveInterval and ClientAliveCountMax are configured
SSHD_CONFIG="/etc/ssh/sshd_config"
...
# Task 4.2.10: Ensure sshd DisableForwarding is enabled
...
# Task 4.2.11: Ensure sshd HostbasedAuthentication is disabled
...
# Task 4.2.12: Ensure sshd IgnoreRhosts is enabled
...
# Task 4.2.13: Ensure sshd LoginGraceTime is configured
...
# Task 4.2.14: Ensure sshd LogLevel is configured
...
# Task 4.2.15: Ensure sshd MaxAuthTries is configured
...
# Task 4.2.16: Ensure sshd MaxStartups is configured
...
# Task 4.2.17: Ensure sshd MaxSessions is configured 
...
# Task 4.2.18: Ensure sshd PermitEmptyPasswords is disabled
...
# Task 4.2.19: Ensure sshd PermitRootLogin is disabled
...
# Task 4.2.20: Ensure sshd PermitUserEnvironment is disabled 
...
# Task 4.2.21: Ensure sshd UsePAM is enabled
...
# Task 4.2.22: Ensure sshd crypto_policy is not set
sed -ri "s/^\s*(CRYPTO_POLICY\s*=.*)$/# \1/" /etc/sysconfig/sshd
systemctl reload sshd

# Task 4.3.1: Ensure sudo is installed
dnf install sudo -y

# Task 4.3.2: Ensure sudo commands use pty
edit_sudoers_file() {
    local file="$1"
...
# Task 4.3.3: Ensure sudo log file exists
edit_sudoers_file() {
    local file="$1"
...
# Task 4.3.4: Ensure users must provide password for escalation
...
# Task 4.3.5: Ensure re-authentication for privilege escalation is not disabled globally
...
# Task 4.3.6: Ensure sudo authentication timeout is configured correctly
...
# Task 4.3.7: Ensure access to the su command is restricted
GROUP_NAME="root"
...
# Task 4.4.1.1: Ensure latest version of pam is installed
dnf upgrade pam -y

# Task 4.4.1.2: Ensure latest version of authselect is installed
dnf install authselect -y
dnf upgrade authselect -y

# Task 4.4.2: Configure authselect
authselect create-profile custom-profile -b sssd

# Task 4.4.2.1: Ensure active authselect profile includes pam modules
authselect select custom/custom-profile --backup=PAM_CONFIG_BACKUP --force

# Task 4.4.2.2: Ensure pam_faillock module is enabled
...
# Task 4.4.2.3: Ensure pam_pwquality module is enabled
...
# Task 4.4.2.4: Ensure pam_pwhistory module is enabled
...
# Task 4.4.2.5: Ensure pam_unix module is enabled
l_module_name="unix"
...
# Task 4.4.3.1.1: Ensure password failed attempts lockout is configured
{
...
# Task 4.4.3.1.2: Ensure password unlock time is configured
for l_pam_file in system-auth password-auth; do
...
# Task 4.4.3.1.3: Ensure password failed attempts lockout includes root account
for l_pam_file in system-auth password-auth; do
    l_authselect_file="/etc/authselect/$(head -1 /etc/authselect/authselect.conf | grep -oP 'custom/\S+')/$l_pam_file"
        if [[ -f "$l_authselect_file" ]]; then
...
# Task 4.4.3.2.1: Ensure password number of changed characters is configured
for l_pam_file in system-auth password-auth; do
    l_authselect_file="/etc/authselect/$(head -1 /etc/authselect/authselect.conf | grep 'custom/')/$l_pam_file"
...
# Task 4.4.3.2.2: Ensure password length is configured
for l_pam_file in system-auth password-auth; do
    l_authselect_file="/etc/authselect/$(head -1 /etc/authselect/authselect.conf | grep 'custom/')/$l_pam_file"
...
# Task 4.4.3.2.3: Ensure password complexity is configured
for l_pam_file in system-auth password-auth; do
    l_authselect_file="/etc/authselect/$(head -1 /etc/authselect/authselect.conf | grep 'custom/')/$l_pam_file"
...
# Task 4.4.3.2.4: Ensure password same consecutive characters is configured
for l_pam_file in system-auth password-auth; do
    l_authselect_file="/etc/authselect/$(head -1 /etc/authselect/authselect.conf | grep 'custom/')/$l_pam_file"
...
# Task 4.4.3.2.5: Ensure password maximum sequential characters is configured
for l_pam_file in system-auth password-auth; do
    l_authselect_file="/etc/authselect/$(head -1 /etc/authselect/authselect.conf | grep 'custom/')/$l_pam_file"
...
# Task 4.4.3.2.6: Ensure password dictionary check is enabled
for l_pam_file in system-auth password-auth; do
    l_authselect_file="/etc/authselect/$(head -1 /etc/authselect/authselect.conf | grep 'custom/')/$l_pam_file"
...
# Task 4.4.3.2.7: Ensure password quality is enforced for the root user
printf '\n%s\n' "enforce_for_root" >> /etc/security/pwquality.conf.d/50-pwroot.conf

# Task 4.4.3.3.1: Ensure password history remember is configured
for l_pam_file in system-auth password-auth; do
    l_authselect_file="/etc/authselect/$(head -1 /etc/authselect/authselect.conf | grep 'custom/')/$l_pam_file"
...
# Task 4.4.3.3.2: Ensure password history is enforced for the root user 
echo "enforce_for_root" >> /etc/security/pwhistory.conf

# Task 4.4.3.3.3: Ensure pam_pwhistory includes use_authtok
l_pam_profile="$(head -1 /etc/authselect/authselect.conf)"
if grep -Pq -- '^custom\/' <<< "$l_pam_profile"; then
    l_pam_profile_path="/etc/authselect/$l_pam_profile"
else
...
# Task 4.4.3.4.1: Ensure pam_unix does not include nullok
l_module_name="unix"
...
# Task 4.4.3.4.2: Ensure pam_unix does not include remember
l_pam_profile="$(head -1 /etc/authselect/authselect.conf)"
if grep -Pq -- '^custom\/' <<< "$l_pam_profile"; then
  l_pam_profile_path="/etc/authselect/$l_pam_profile"
else
...
# Task 4.4.3.4.3 Ensure pam_unix includes a strong password hashing algorithm
l_pam_profile="$(head -1 /etc/authselect/authselect.conf)"
if grep -Pq -- '^custom\/' <<< "$l_pam_profile"; then
  l_pam_profile_path="/etc/authselect/$l_pam_profile"
else
...
# Task 4.4.3.4.4: Ensure pam_unix includes use_authtok
l_pam_profile="$(head -1 /etc/authselect/authselect.conf)"
if grep -Pq -- '^custom\/' <<< "$l_pam_profile"; then
  l_pam_profile_path="/etc/authselect/$l_pam_profile"
else
...
# Task 4.5.1.1: Ensure strong password hashing algorithm is configured
sed -ri 's/^ENCRYPT_METHOD\s+.*/ENCRYPT_METHOD YESCRYPT/' /etc/login.defs

# Task 4.5.1.2: Ensure password expiration is configured
sed -ri 's/^PASS_MAX_DAYS\s+.*/PASS_MAX_DAYS 365/' /etc/login.defs

# Task 4.5.1.3: Ensure password expiration warning days is configured
sed -ri 's/^PASS_WARN_AGE\s+.*/PASS_WARN_AGE 7/' /etc/login.defs

# Task 4.5.1.4: Ensure inactive password lock is configured
#useradd -D -f 45
#awk -F: '($2~/^\$.+\$/) {if($7 > 45 || $7 < 0)system ("chage --inactive 45 " $1)}' /etc/shadow

# Task 4.5.1.5: Ensure all users last password change date is in the past
...
# Task 4.5.2.1: Ensure root is the only GID 0 account
usermod -g 0 root
groupmod -g 0 root

# Task 4.5.2.2: Ensure root user umask is configured
...
# Task 4.5.3.1: Ensure nologin is not listed in /etc/shells
sed -i '/nologin/d' /etc/shells

# Task 4.5.3.2: Ensure default user shell timeout is configured
...
# Task 4.5.3.3: Ensure default user umask is configured
file_umask_chk() {
