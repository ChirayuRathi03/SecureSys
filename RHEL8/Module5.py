...
try:
    journal_remote_status = subprocess.run(
    ...
    if journal_remote_status == expected_output:
        actual_output = "masked"
        true_counter += 1
        true_tasks.append(task_no)
        task_data.append([task_no, "True", expected_output, actual_output])
    else:
        actual_output = f"systemd-journal-remote.socket status: {journal_remote_status}"
        false_counter += 1
        false_tasks.append(task_no)
        task_data.append([task_no, "False", expected_output, actual_output])
except Exception as e:
...
try:
    ...
    if actual_output1.startswith(expected_output_prefix1):
        true_counter += 1
        true_tasks.append(task_no)
        task_data.append([task_no, "True", f"{expected_output_prefix1}x", actual_output])
    else:
        false_counter += 1
        false_tasks.append(task_no)
        task_data.append([task_no, "False", f"{expected_output_prefix1}x", actual_output])
except Exception as e:
    false_counter += 1
    false_tasks.append(task_no)
    task_data.append([task_no, "False", f"{expected_output_prefix1}x", f"Error: {str(e)}"])
