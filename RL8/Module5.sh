...
# Task 5.1.1.1: Ensure rsyslog is installed
dnf install rsyslog -y

# Task 5.1.1.2: Ensure rsyslog service is enabled
...
# Task 5.1.1.3: Ensure journald is configured to send logs to rsyslog
if systemctl is-active --quiet rsyslog; then
...
# Task 5.1.1.4: Ensure rsyslog default file permissions are configured
...
# Task 5.1.1.5: Ensure logging is configured (manual)

# Task 5.1.1.6: Ensure rsyslog is configured to send logs to a remote log host (manual)
...
# Task 5.1.1.7: Ensure rsyslog is not configured to receive logs from a remote client
patterns=(
    "module(load=\"imtcp\")"
...
# Task 5.1.2.1.1: Ensure systemd-journal-remote is installed (manual)
dnf install systemd-journal-remote -y

# Task 5.1.2.1.2: Ensure systemd-journal-remote is configured (manual)

# Task 5.1.2.1.3: Ensure systemd-journal-remote is enabled (manual)
systemctl unmask systemd-journal-upload.service
systemctl --now enable systemd-journal-upload.service

# Task 5.1.2.1.4: Ensure journald is not configured to receive logs from a remote client
systemctl --now mask systemd-journal-remote.socket 

# Task 5.1.2.2: Ensure journald service is enabled
systemctl unmask systemd-journald.service
systemctl start systemd-journald.service

# Task 5.1.2.3: Ensure journald is configured to compress large log files
grep -q '^Compress=' /etc/systemd/journald.conf && sed -i '/^Compress=/c\Compress=yes' /etc/systemd/journald.conf || echo 'Compress=yes' >> /etc/systemd/journald.conf
systemctl restart systemd-journald.service

# Task 5.1.2.4 Ensure journald is configured to write logfiles to persistent disk 
grep -q '^Storage=' /etc/systemd/journald.conf && sed -i '/^Storage=/c\Storage=persistent' /etc/systemd/journald.conf || echo 'Storage=persistent' >> /etc/systemd/journald.conf
systemctl restart systemd-journald.service

# Task 5.1.2.5: Ensure journald is not configured to send logs to rsyslog
sed -i '/^ForwardToSyslog=yes/d' /etc/systemd/journald.conf
systemctl restart systemd-journald.service

# Task 5.1.2.6: Ensure journald log rotation is configured per site policy (manual)
...
# Task 5.1.3: Ensure logrotate is configured (manual)

# Task 5.1.4: Ensure access to all logfiles has been configured
...
# Task 5.2.1.1: Ensure auditd packages are installed
dnf install audit audit-libs -y

# Task 5.2.1.2: Ensure auditing for processes that start prior to auditd is enabled
...
# Task 5.2.1.3: Ensure audit_backlog_limit is sufficient
...
# Task 5.2.1.4: Ensure auditd service is enabled and active
systemctl unmask auditd
systemctl enable auditd
systemctl start auditd

# Task 5.2.2.1: Ensure audit log storage size is configured
...
# Task 5.2.2.2: Ensure audit logs are not automatically deleted 
auditd_conf="/etc/audit/auditd.conf"
temp_file=$(mktemp)
...
# Task 5.2.2.3: Ensure system is disabled when audit logs are full
auditd_conf="/etc/audit/auditd.conf"
...
# Task 5.2.2.4: Ensure system warns when audit logs are low on space
auditd_conf="/etc/audit/auditd.conf"
...
# Task 5.2.3.1: Ensure changes to system administration scope (sudoers) is collected
...
# Task 5.2.3.2: Ensure actions as another user are always logged
...
# Task 5.2.3.3: Ensure events that modify the sudo log file are collected
...
# Task 5.2.3.4: Ensure events that modify date and time information are collected
...
# Task 5.2.3.5: Ensure events that modify the system's network environment are collected
...
# Task 5.2.3.6: Ensure use of privileged commands are collected
...
# Task 5.2.3.7: Ensure unsuccessful file access attempts are collected
UID_MIN=$(awk '/^\s*UID_MIN/{print $2}' /etc/login.defs)
...
# Task 5.2.3.8: Ensure events that modify user/group information are collected
...
# Task 5.2.3.9: Ensure discretionary access control permission modification events are collected
UID_MIN=$(awk '/^\s*UID_MIN/{print $2}' /etc/login.defs)
...
# Task 5.2.3.10: Ensure successful file system mounts are collected 
UID_MIN=$(awk '/^\s*UID_MIN/{print $2}' /etc/login.defs)
...
# Task 5.2.3.11: Ensure session initiation information is collected
...
# Task 5.2.3.12: Ensure login and logout events are collected
...
# Task 5.2.3.13: Ensure file deletion events by users are collected
UID_MIN=$(awk '/^\s*UID_MIN/{print $2}' /etc/login.defs)
...
# Task 5.2.3.14: Ensure events that modify the system's Mandatory Access Controls are collected
...
# Task 5.2.3.15: Ensure successful and unsuccessful attempts to use the chcon command are collected
UID_MIN=$(awk '/^\s*UID_MIN/{print $2}' /etc/login.defs)
...
# Task 5.2.3.16: Ensure successful and unsuccessful attempts to use the setfacl command are collected
UID_MIN=$(awk '/^\s*UID_MIN/{print $2}' /etc/login.defs)
RULE_FILE="/etc/audit/rules.d/50-perm_chng.rules"
...
# Task 5.2.3.17: Ensure successful and unsuccessful attempts to use the chacl command are collected
UID_MIN=$(awk '/^\s*UID_MIN/{print $2}' /etc/login.defs)
RULE_FILE="/etc/audit/rules.d/50-perm_chng.rules"
...
# Task 5.2.3.18: Ensure successful and unsuccessful attempts to use the usermod command are collected
UID_MIN=$(awk '/^\s*UID_MIN/{print $2}' /etc/login.defs)
...
# Task 5.2.3.19: Ensure kernel module loading unloading and modification is collected
UID_MIN=$(awk '/^\s*UID_MIN/{print $2}' /etc/login.defs)
...
# Task 5.2.3.20: Ensure the audit configuration is immutable
printf '\n-e 2' > /etc/audit/rules.d/99-finalize.rules
augenrules --load

# Task 5.2.3.21: Ensure the running and on disk configuration is the same (manual)

# Task 5.2.4.1: Ensure the audit log file directory mode is configured 
...
# Task 5.2.4.2: Ensure audit log files mode is configured
...
# Task 5.2.4.3: Ensure audit log files owner is configured
...
# Task 5.2.4.4: Ensure audit log files group owner is configured
...
# Task 5.2.4.5: Ensure audit configuration files mode is configured
find /etc/audit/ -type f \( -name '*.conf' -o -name '*.rules' \) -exec chmod u-x,g-wx,o-rwx {} +

# Task 5.2.4.6: Ensure audit configuration files owner is configured
find /etc/audit/ -type f \( -name '*.conf' -o -name '*.rules' \) ! -user root -exec chown root {} +

# Task 5.2.4.7: Ensure audit configuration files group owner is configured
find /etc/audit/ -type f \( -name '*.conf' -o -name '*.rules' \) ! -group root -exec chgrp root {} +

# Task 5.2.4.8: Ensure audit tools mode is configured
chmod go-w /sbin/auditctl /sbin/aureport /sbin/ausearch /sbin/autrace /sbin/auditd /sbin/augenrules

# Task 5.2.4.9: Ensure audit tools owner is configured
chown root /sbin/auditctl /sbin/aureport /sbin/ausearch /sbin/autrace /sbin/auditd /sbin/augenrules

# Task 5.2.4.10: Ensure audit tools group owner is configured
chgrp root /sbin/auditctl /sbin/aureport /sbin/ausearch /sbin/autrace /sbin/auditd /sbin/augenrules

# Task 5.3.1: Ensure AIDE is installed
dnf install aide -y
aide --init
mv /var/lib/aide/aide.db.new.gz /var/lib/aide/aide.db.gz

# Task 5.3.2: Ensure filesystem integrity is regularly checked:
#crontab -u root -e
...
# Task 5.3.3: Ensure cryptographic mechanisms are used to protect the integrity of audit tools
